...
def _record_kanban_budget_exhausted(
    ...
    Also passes ``expected_run_id`` (from ``HERMES_KANBAN_RUN_ID``) through to
    ``_record_task_failure`` so a stale worker cannot clobber the successor
    (#03c16b25). Absent or unparseable, the CAS is skipped.
    """
    try:
