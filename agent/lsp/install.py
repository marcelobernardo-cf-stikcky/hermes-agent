...
def _usable_binary(path: Path, *, is_windows: Optional[bool] = None) -> bool:
...
def _first_existing(*bases: Path, is_windows: Optional[bool] = None) -> Optional[Path]:
    ...
    return next((c for base in bases for c in _native_binary_candidates(base, is_windows=is_windows) if _usable_binary(c, is_windows=is_windows)), None)
...
def _existing_binary(name: str, *, is_windows: Optional[bool] = None) -> Optional[str]:
    ...
    for staged in (c for base in bases for c in _native_binary_candidates(base, is_windows=win)):
        if _usable_binary(staged, is_windows=win) and os.access(staged, os.X_OK):
            return str(staged)
