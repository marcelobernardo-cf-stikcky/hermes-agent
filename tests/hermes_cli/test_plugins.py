...
class TestForceReloadSymmetry:
    ...
    def test_pre_tool_call_concurrent_calls_without_identity_fail_closed(self, monkeypatch):
        """An uncorrelated concurrent policy call cannot be safely matched, so it fails closed."""
        import time
        ...
        second.start()
        time.sleep(0.05)
        assert results["allow"] == [{"action": "block", "message": _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE}]
        release.set()
        ...
        assert results == {
            ...
            "allow": [{"action": "block", "message": _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE}],
        }
        assert seen == [{"deny": True}]
