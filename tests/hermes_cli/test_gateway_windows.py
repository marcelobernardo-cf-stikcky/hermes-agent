...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    """The .vbs launcher must avoid cmd.exe and supervise a hidden child.

    The console ``python.exe`` child is hidden by WScript (rather than using
    pythonw.exe, whose descendants can flash their own consoles).
    """
    monkeypatch.setattr(
        gateway_windows,
