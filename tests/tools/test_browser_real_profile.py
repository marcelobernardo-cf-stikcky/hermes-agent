...
def _daemon_alive_by_default(monkeypatch):
    """Make real-profile daemon probes deterministic in tests."""
    import pm
    import pm.install
    import hermes_cli.browser_runtime as browser_runtime

    def _env_for(*_names, base_env=None):
        return dict(base_env or {})

    monkeypatch.setattr(bt_real_profile, "_agent_browser_daemon_alive", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(bt_real_profile, "_agent_browser_get_cdp", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(bt_real_profile, "_agent_browser_close_session", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(pm.install, "env_for", _env_for)
    monkeypatch.setattr(pm, "env_for", _env_for, raising=False)
