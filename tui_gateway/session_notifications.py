...
# _KANBAN_NOTIFY_KINDS is imported from gateway.kanban_watchers_notifier (one source of truth): silent kinds
# (archived/unblocked) are claimed too so the cursor advances past them and can't wedge a later event behind an
# unclaimed row. Not redefined here — a local copy silently drifts (it dropped review_requested once).
