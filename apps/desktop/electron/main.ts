...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  // A fork's `origin` is the user's OWN repo: it never advances on its own, so checking it
  // reports "up to date" forever while the official repo moves on. Prefer `upstream` when that
  // remote exists - the same source `hermes update` already treats as a fork's real source.
  const upstreamUrl = (await runGit(['remote', 'get-url', 'upstream'], { cwd: updateRoot }))
  const checkUrl = upstreamUrl.code === 0 && upstreamUrl.stdout.trim() ? upstreamUrl.stdout.trim() : originUrl
  const slug = githubRepoSlug(checkUrl)
