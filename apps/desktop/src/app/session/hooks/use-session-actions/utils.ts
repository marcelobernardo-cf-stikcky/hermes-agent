...
export function overlayConcurrentMessageChanges(
  ...
  for (const addition of additions) {
    ...
    // Without a right-hand live neighbor, the REST tail is authoritative and
    // the concurrent row belongs after it. With one, insert immediately before
    // that neighbor to preserve the live transcript order.
    const insertionIndex = nextKnown
